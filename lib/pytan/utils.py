...
class SplitStreamHandler(logging.Handler):
    """Custom :class:`logging.Handler` class.

    Sends all messages that are logging.INFO and below to STDOUT,
    and all messages that are logging.WARNING and above to STDERR
    """
    ...
    def emit(self, record):
        ...
        try:
            ...
            fs = "%s\n"
            try:
                is_unicode = isinstance(msg, uni_type)
...
def change_console_format(debug=False):
    """Change the logging format for console handler.

    Will use :data:`pytan.constants.DEBUG_FORMAT` or :data:`pytan.constants.INFO_FORMAT`.
...
def set_log_levels(loglevel=0):
    ...
    loglevel : int, optional
        * loglevel to match against each item in :data:`pytan.constants.LOG_LEVEL_MAPS`
          each item that is greater than or equal to loglevel will have the according loggers
          set to their respective levels identified there-in.
    """
    if loglevel >= 20:
...
def get_all_pytan_loggers():
    ...
    Filter for loggers that exist in :data:`pytan.constants.LOG_LEVEL_MAPS`.
...
def extract_selector(s):
    ...
    ----------
    s : str
        * A str that may or may not have a selector in the beginning in the form of:
         id:, name:, or :hash -- if no selector found, name will be assumed as the default selector
...
def parse_defs(defname, deftypes, strconv=None, empty_ok=True, defs=None, **kwargs):
    """Parse and validate defs into new_defs.
...
def val_sensor_defs(sensor_defs):
    ...
    Ensures each sensor definition has a selector, and if a sensor definition has
    a params, options, or filter key, that each key is valid
...
def val_q_filter_defs(q_filter_defs):
    ...
    Ensures each question filter definition has a selector,
    and if a question filter definition has a filter key, that key is valid
...
def build_group_obj(q_filter_defs, q_option_defs):
    ...
    group_obj : :class:`taniumpy.object_types.group.Group`
        * Group object with list of :class:`taniumpy.object_types.filter.Filter` built from
          `q_filter_defs` and `q_option_defs`
    """
...
def build_param_objlist(obj, user_params, delim='', derive_def=False, empty_ok=False):
    ...
    derive_def : bool, optional
        * False: Do not derive default values, and throw a :exc:`pytan.exceptions.HandlerError`
          if user did not supply a value for a given parameter
...
def copy_obj(obj, skip_attrs=None):
    ...
    [
        ...
        for a in vars(obj)
        if getattr(obj, a, None) is not None and
        a not in skip_attrs
    ]
    return new_obj
...
def copy_package_obj_for_action(obj, skip_attrs=None):
    ...
    skip_attrs : list of str
        * default: None
        * list of attribute str's to skip copying over to new object, default if None:
          ['id', 'deleted_flag', 'available_time', 'creation_time', 'modification_time', 'source_id']
...
def log_session_communication(h):
    """Use :func:`xml_pretty` to pretty print the last request and response bodies to the logging system.

    Gets them from the session object in handler `h`.
